...
class AgenteSonorizacao(Agent):
    def __init__(self, aid, f):
        super(AgenteSonorizacao, self).__init__(aid=aid)
        self.f = f
    ...
    def launch_subscriber_protocol(self):
        ...
        message.add_receiver(AID('cortina'))
        self.protocol = SubscriberProtocol(self, message)
        self.behaviours.append(self.protocol)
        self.protocol.on_start()

    def on_start(self):
        super(AgenteSonorizacao, self).on_start()
    ...
    def react(self, message):
        ...
        if self.f.filter(message):
            if f'{message.content}'.startswith("Apagando"):
                self.send_message("Estabelecendo o audio...", "projetor")
            
            if f'{message.content}'.startswith("Filme"):
                self.send_message("Desligando o audio...", "iluminacao")